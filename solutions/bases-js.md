...
## Points clés :
- L'utilisation de `find()` pour rechercher un client unique
- La vérification `if (!client)` pour gérer les cas d'erreur
- L'utilisation de `reduce()` pour calculer le total
- Le formatage avec `toFixed(2)` pour l'affichage des prix
- Le return true/false pour indiquer le succès de l'opération
