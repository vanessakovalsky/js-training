...
export class ProduitsController {
  ...
  async findAll(
    ...
    // Si page est fourni, utiliser la pagination
    if (page) {
      const result = await this.produitsService.findAllPaginated(
        parseInt(page),
    ...
    if (search) {
      const produits = await this.produitsService.search(search);
      return {
        succes: true,
        data: produits,
